...
| --- | --- | --- | --- | --- |
| BE-01 | C | Supervisor restarts coroutines that *return* normally with no delay → event-loop spin; also makes polling unstoppable (poller swallows cancel) and `disconnect()`/`shutdown()` hang | `_task_supervisor.py:26-39`, `telnet_client.py:485,589`, `driver.py:1484` | 1 |
| BE-02 | C | `await set_matrix_device(...)` on a sync function → every UC setup/reconfigure returns an error; macro CEC sender never wired | `driver.py:2015` | 1 |
| BE-03 | H | Output/input/cable status caches never expire (cache time written, never read) → sensors frozen | `orei_matrix.py:1023,1053,1139` | 1 |
| BE-04 | H | `connected` not cleared on timeouts/transport errors; no re-login on session expiry; reconnect never triggers | `orei_matrix.py:~366` | 1 |
| BE-05 | H | Login treated as success when `comhead == "login"` (echoed even on bad password) | `orei_matrix.py:251` | 1 |
| BE-06 | H | Reconnect loop cancels itself via CONNECTED → `_stop_reconnection()`; Telnet never reconnected, polling not restarted | `driver.py:~1777` | 1 (still open after WP-A4 part 2: since HIL-12 one failed status read no longer drops the link, so `tests/uc/test_state_sync.py::test_live_updates_resume_after_a_transient_failure` now triggers it with a real short link loss (`drop_http`); the strict xfail still reproduces) |
| BE-07 | H | Telnet completion logic contradicts itself (waits for E00/E01, then treats them as failure); set commands always wait full 5 s | `telnet_client.py:403-426,884,933` | 1 (HIL) |
| BE-08 | M | `conn_state` unbound in poller when a sensor is not configured → poll aborts forever | `driver.py:1338` | 1 |
| BE-09 | M | Standby/shutdown disconnect emits DISCONNECTED → reconnect loop restarts ~5 s into standby | `driver.py:~1829` | 1 |
| BE-10 | M | Setup creates a new `OreiMatrix` without disposing the old one (session, socket, listeners leak; old events drive reconnect) | `driver.py:~1884` | 1 |
| BE-11 | M | `_connect_telnet` replaces client without disconnecting the old one; concurrent `connect()` races; `_send_raw` error path leaves writer open | `orei_matrix.py:139`, `telnet_client.py` | 1 |
| BE-12 | M | Write methods report success on HTTP 200 regardless of `result`; `switch_input_to_all` returns True on failure | `orei_matrix.py:582-585,864,1791-1896` | 1 |
...
| ID | Sev | Finding | Location | Phase |
| --- | --- | --- | --- | --- |
| API-01 | C | `matrix_device.switch()` does not exist → every scene profile step and route shortcuts fail | `scene_execution.py:161`, `system_shortcuts.py:697,702` | 2 / WP-C1 ✅ fixed: `switch_input` with the result checked; tests use a `create_autospec(OreiMatrix)` double (the old ones mocked `switch`). Proof: `tests/sim/test_sim_scenes_v2.py::test_execute_profile_and_route_steps`, scenarios `scenes.run_protected`, `shortcuts.route_*` |
| API-02 | C | `profile_manager._save()` does not exist → 500, steps skipped, no history | `scene_execution.py:455` | 2 / WP-C1 ✅ fixed: `save()`. Proof: `tests/test_scene_execution.py::test_execution_is_logged_on_the_profile_and_saved`, scenario `scenes.run_profile_macro_preset` (history and profile log read back through the hub) |
| API-03 | H | Macro step execution calls `.get()` on a dataclass and expects `input:3` targets (macros use `input_1`) | `scene_execution.py:190-195` | 2 / WP-C1 ✅ fixed: a profile's macros run through `MacroManager.execute_macro`. Proof: `test_execute_profile_step_runs_the_profile_macros_through_the_macro_manager` (CEC frames in the simulator log), scenario `scenes.run_profile_macro_preset` |
| API-04 | H | Scene overrides apply defaults instead of "leave unchanged" (disabling input override routes Input 1) | `scene_execution.py:111-127` | 2 / WP-C1 ✅ fixed: an active override (value `true`) leaves that setting unchanged; `false` is not an override. Proof: `test_overrides_leave_the_setting_unchanged`, scenarios `scenes.override_leaves_setting`, `scenes.run_profile_macro_preset` |
| API-05 | H | `p.id` on dicts from `list_profiles()` → `POST/PUT /api/v2/scenes` 500 whenever profiles exist | `scenes_v2.py:126,193` | 2 / WP-C1 ✅ fixed: profiles looked up with `get_profile()`. Proof: `tests/sim/test_sim_scenes_v2.py` create/update tests, scenario `scenes.edit_with_profile_step` |
| API-06 | H | `power_off_all` powers off the matrix 8× | `system_shortcuts.py:705-707` | 2 / WP-C1 ✅ fixed: one `set poweronoff`. Proof: scenario `shortcuts.power_off_all` (`count=1`), `test_power_off_all_step_powers_the_matrix_off_once` |
| API-07 | M | LCD timeout mode map in shortcuts contradicts device mapping ("Always on" sets 60 s) | `system_shortcuts.py:22-34` vs `orei_matrix.py:1972` | 2 (HIL) / WP-A4 part 2: shortcut map uses the device codes (0 off, 1 always, 2/3/4 = 15/30/60 s; read back as `get system status.mode`, 3 = 30 s captured); the old `{"time": N}` payload is rejected, the hub sends `{"lcd on time": N}` (proven with HIL-09). The shortcut label "LCD: 10s" (sets 15 s) needs a UI-approved rename. WP-C1: renamed "LCD: 15s" (owner-approved; key `lcd_timeout_10s` kept for pins/Flic; a stored label equal to the old default is migrated on load), proven by scenario `shortcuts.lcd_15s`; close once the owner approves the visual diff of the affected catalog entries |
| API-08 | M | Boolean results ignored; `execute_scene` returns 200/`success:true` on failure; profile recall 200 when all outputs fail | `system_shortcuts.py`, `scenes_v2.py:275`, `profiles.py` | 2 / WP-C1 ✅ fixed: scene execute 200 / 207 (some steps failed) / 500 (none succeeded) / 404 (unknown) with `step_results`; recall 200 / 207 / 500 with `failed_outputs`; shortcuts fail on a refused write. Proof: scenarios `scenes.partial_failure`, `scenes.run_rejected`, `scenes.run_unknown`, `profiles.recall_rejected`, `shortcuts.mute_rejected` |
...
| API-11 | M | Every `/api/status` cache hit spawns an untracked, non-deduplicated refresh task (4 sequential matrix calls) | `core.py:225-236` | 1 |
...
| API-13 | M | `update_scene` mutates in memory before validating → corrupt in-memory state | `scene_manager.py:590-616` | 2 / WP-C1 ✅ fixed: changes go to a copy that replaces the scene only after validation and save; overrides are validated too. Proof: `test_invalid_update_leaves_the_scene_unchanged`, scenario `scenes.invalid_edit_rejected` |
| API-14 | M | Custom preset save re-routes live TVs, restores from stale routing, no lock | `control.py:383-424` | 2 / WP-C1 ✅ fixed within current behaviour (it still re-routes the live displays briefly; DI-4 decides that): one save at a time, routing read fresh from the matrix (refused if unreadable), no save if the temporary routing failed, only the changed outputs routed back. Proof: `tests/sim/test_sim_preset_save.py`, scenario `presets.save_custom_mapping` |
...
| API-23 | H | `POST` scene CEC auto-resolve calls `resolve_scene_cec_config(profile)` but the resolver takes `(active_inputs, active_outputs, status)` and returns a dict (then `.to_dict()` is called on it) → endpoint always returns 500 (found by mypy in Phase 0) | `rest_api/scenes.py:~257`, `cec_resolver.py` | 2 / WP-C1 ✅ fixed: resolver called with the profile's active inputs/outputs and the matrix output status. Proof: `test_scene_cec_auto_resolve`, scenario `profiles.cec_auto_resolve` |
...
| --- | --- | --- | --- | --- | --- |
| VAL-01 | H | Profile recall never applies audio mute, HDR or HDCP: it checks `hasattr(matrix_device, "set_audio_mute" / "set_hdr_mode" / "set_hdcp_mode")`, but the methods are named `set_output_*`, so the settings are silently skipped and recall still returns 200 | `rest_api/profiles.py:322-329` | `profiles.recall_output_settings` | 2 / WP-C1 ✅ fixed: recall calls `set_output_audio_mute` / `set_output_hdr` (API value, mapped to the device code) / `set_output_hdcp` and checks each answer; the scenario passes (`count=1` for each command) |
| VAL-02 | M | Modular mode (`run.py`, the target architecture) never wires the macro CEC sender, so CEC macros, profile power macros and scene macro steps fail with "CEC sender not configured" while recall reports success | `run.py:88-106`, `rest_api/utils.py:461` | `profiles.recall_power_macro` | 2 / WP-C1 ✅ fixed without touching `run.py`: `set_matrix_device()` installs a default macro CEC sender (`rest_api.utils.matrix_cec_sender`, `OreiMatrix.send_cec`) in every mode; the UC driver still installs its own afterwards. Scenarios `profiles.recall_power_macro`, `macros.run_volume_up`, `scenes.run_shortcut_and_macro` pass |
...
- [ ] **Scenes & shortcuts** (API-01…08, API-13, API-14, API-23): `switch_input`; `save()`; macro steps via `MacroManager` with the real target format; overrides mean "leave unchanged"; `list_profiles` usage; `power_off_all` once; LCD map; honest status codes (`207`/`500` with per-step results); validate-then-mutate; custom preset save gets a routing lock and restores from fresh (not cached) routing — any change to *what* it does waits for DI-4. Full `/api/v2/scenes` test suite. All fixes preserve current behaviour and data formats (D5).
  - WP-C1: all fixed with regression tests and V2 scenarios (see the register rows) and the `/api/v2/scenes` suite is `tests/sim/test_sim_scenes_v2.py`; the box stays open until the owner approves the visual diff of the "LCD: 15s" label (API-07).
