...
| VAL-07 | M | A profile is applied differently by recall and by a scene's profile step: recall routes a disabled output, turns its stream off and runs `power_on_macro`; the scene step skips disabled outputs, never changes the stream state (an "enabled" override has no effect) and runs the quick-access `macros` list instead, so `scene_movienight01` sends `macro_tv_on` twice (concrete instance of API-19) | `rest_api/profiles.py` `handle_recall_profile`, `scene_execution.py` `_execute_profile` | `scenes.run_profile_macro_preset` (2 CEC power-on frames) | 4 / DI-2 (profile = desired state) |
| VAL-08 | M | A macro's own `continue_on_error` is never honoured: every caller (macro execute endpoint, profile power macro, scene macro steps, profile macros) uses the default `False`, so `macro_all_off` (`continue_on_error: true`) stops at the first refused target | `cec_macros.py` `execute_macro`, `rest_api/macros.py` | code reading; no scenario yet (needs a CEC refusal on one target) | 2 / WP-C1 follow-up |
| VAL-09 | L | F-DOM-013 claims a scene "delay" step that does not exist: `VALID_STEP_TYPES` is profile, system_action, macro, and a delay step is rejected with 400. Drop the claim or add the step type (feature change: discuss first, D5) | `scene_manager.py`, `features.yaml` F-DOM-013 | `tests/sim/test_sim_scenes_v2.py::test_create_rejects_invalid_scenes` | discuss (D5) |
| VAL-10 | M | Scene conflict detection never reports a conflict: `detect_conflicts()` keeps only the first value per output setting and the "different value" branch is `pass`, so `POST /api/v2/scenes/{id}/validate` always answers `has_conflicts: false` | `scene_manager.py` `detect_conflicts` | code reading; `TestDetectConflicts` has no conflicting case | 2 / WP-C1 follow-up |
| VAL-11 | L | No UI feedback for a scene or profile that ran partly (HTTP 207, `success: false`): the UI only toasts on success, so a partial failure looks like nothing happened (before WP-C1 the hub claimed success) | `web/js/components/settings-drawer.js:367`, `web/js/utils/dashboard-manager.js:909` | `scenes.partial_failure` (the 207 body the UI receives) | 2 / WP-E1 (with UI-01) |
