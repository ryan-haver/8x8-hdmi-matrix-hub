...
| HA-14 | M | HACS validation fails: no `hacs.json`, no GitHub repository topics (owner action: add e.g. `home-assistant`, `hacs`, `hdmi-matrix`), no brand assets (icon/logo via the HA brands repo or bundled with the integration); licence check passes once the full LICENSE is on `main` | repo root, GitHub settings | 2 / WP-D1: `hacs.json` added (Home Assistant 2025.1, D8), topics are set, the HACS action passes every check except brands; open until brand assets exist (`custom_components/hdmi_matrix/brand/icon.png` or the HA brands repo; owner) |
...
| WP-D1 | D | Home Assistant fixes + real-HA-container E2E (HA-01–15) | WP-V1 | HA features at V3 in a real HA container | done on `wp-d1-home-assistant`: HA-01…13, 15 fixed; F-HA-001…013, 015…018 at V3 (HA 2026.9.3 and 2025.1.4); HA-14 brand assets open |
