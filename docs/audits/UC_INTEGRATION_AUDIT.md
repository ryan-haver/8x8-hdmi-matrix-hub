...
**Status (WP-B2):** the wave-2 driver fixes landed (UC-01, UC-04, UC-06, UC-07, UC-17, UC-19, UC-22, the UC-05 probe-before-swap part, the UC-10 first-toggle bug; BE-02, BE-06, BE-08, BE-09, BE-10, BE-21). Their strict xfails pass and were removed, and new harness tests cover the recovery, standby/wake, partial subscriptions, reconfigure to another matrix and the command parameters. Renames the driver sees (Remote `connect`, start-up, setup) now replace the subscribed entities in place, which also fixed UC-08's reconnect case. Six strict xfails remain for later work packages: UC-02, UC-03, UC-08 (live web-app rename), UC-09 (2), UC-14. Driver behaviour that follows from the fixes: CEC remotes and media players start `UNKNOWN` (the hub cannot read a TV's or source's power; it tracks the last power command it sent, and a toggle from unknown powers on); sensors start `UNKNOWN`; the media player `source` is empty until the first poll; subscribing an entity sends its full current state once.
