...
pytestmark = pytest.mark.skipif(
    ...
    reason="Home Assistant is not installed in the testing environment"
)
...
async def test_config_flow_success():
    ...
    with patch(
        "custom_components.orei_matrix.config_flow.async_get_clientsession",
...
async def test_coordinator_update_success():
    ...
    mock_status_resp.json = AsyncMock(return_value={
        "success": True,
        "data": {
            "connected": True,
            "routing": {"1": 2, "2": 3},
    ...
    mock_outputs_resp.json = AsyncMock(return_value={
        "success": True,
        "data": {
            "outputs": [
    ...
    mock_inputs_resp.json = AsyncMock(return_value={
        "success": True,
        "data": {
            "inputs": [
                {"number": 1, "name": "NES", "signalActive": True, "cableConnected": True}
            ]
        }
    })
    ...
    with patch(
        "custom_components.orei_matrix.coordinator.async_get_clientsession",
        return_value=mock_session
    ):
